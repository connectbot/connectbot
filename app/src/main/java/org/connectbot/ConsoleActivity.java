...
public class ConsoleActivity extends Activity {
    ...
    private MenuItem disconnect, copy, copyEdit, paste, portForward, resize, urlscan;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        TerminalView view;
        try {
            view = adapter.getCurrentTerminalView();
        } catch (Exception e) {
            e.printStackTrace();
            view = null;
        }
        ...
        copyEdit = menu.add(R.string.console_menu_copy_edit);
        if (hardKeyboard)
            copyEdit.setAlphabeticShortcut('e');
        copyEdit.setIcon(android.R.drawable.ic_menu_set_as);
        copyEdit.setEnabled(activeTerminal);
        copyEdit.setOnMenuItemClickListener(new OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                startCopyEditMode();
                return true;
            }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // Quick Fix : avoid FC in Genymotion and maybe other devices with physical keyboard.
        // NullPointerException : java.util.ArrayList org.connectbot.service.TerminalManager.getBridges()
        TerminalView view;
        try {
            view = adapter.getCurrentTerminalView();
        } catch (Exception e) {
    ...
    private void startCopyEditMode() {
        TerminalView terminalView = adapter.getCurrentTerminalView();
        VDUBuffer vb = terminalView.bridge.getVDUBuffer();

        int bottom = vb.getRows() - 1;
        int right = vb.getColumns() - 1;
        ...
        while (lines.get(lines.size() - 1).trim().length() == 0)
            lines.remove(lines.size() - 1);

        Intent intent = new Intent(this, CopyEditActivity.class);
        intent.putExtra(CopyEditActivity.ARG_BUFFER, TextUtils.join("\n", lines));
        startActivity(intent);
    }
